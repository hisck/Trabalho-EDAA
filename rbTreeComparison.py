...
for i in range(0, 10):
    ...
    for j in range(0, 100):
        ...
        found_random = RbTree.find(Random(j).randint(0, max_generated_numbers), 0)
        end_mem = psutil.Process(os.getpid()).memory_full_info().uss
        end_time = time.time()
        elapsed_time = end_time - start_time
        deepest_node = RbTree.find_deepest_node()
        print(j)
        stats.append({'time_elapsed': elapsed_time, 'mem_used': end_mem - initial_mem, 'comparison_count': found_random[1], 'height': deepest_node[1]})

    for k in range (0, 3):
        initial_mem = psutil.Process(os.getpid()).memory_full_info().uss
        deepest_node = RbTree.find_deepest_node()
        start_time = time.time()
        found_random = RbTree.find(deepest_node[0].val, 0)
        end_mem = psutil.Process(os.getpid()).memory_full_info().uss
        end_time = time.time()
        elapsed_time = end_time - start_time
        worst_case.append({'time_elapsed': elapsed_time, 'mem_used': end_mem - initial_mem, 'comparison_count': found_random[1], 'height': deepest_node[1]})
