...
for i in range(0, 10):
    ...
    for i in nums:
        AvlTree.insert_value(i)
    for j in range(0, 100):
        initial_mem = psutil.Process(os.getpid()).memory_full_info().uss
        start_time = time.time()
        found_random = AvlTree.search_value(Random(j).randint(0, max_generated_numbers))
        end_mem = psutil.Process(os.getpid()).memory_full_info().uss
        end_time = time.time()
        elapsed_time = end_time - start_time
        deepest_node = AvlTree.find_deepest_node(AvlTree.root)
        print(j)
        stats.append({'time_elapsed': elapsed_time, 'mem_used': end_mem - initial_mem, 'comparison_count': found_random[1], 'height': deepest_node.h})
    for k in range (0, 3):
        initial_mem = psutil.Process(os.getpid()).memory_full_info().uss
        deepest_node = AvlTree.find_deepest_node(AvlTree.root)
        start_time = time.time()
        found_random = AvlTree.search_value(deepest_node.value)
        end_mem = psutil.Process(os.getpid()).memory_full_info().uss
        end_time = time.time()
        elapsed_time = end_time - start_time
        worst_case.append({'time_elapsed': elapsed_time, 'mem_used': end_mem - initial_mem, 'comparison_count': found_random[1], 'height': deepest_node.h})
    ...
    height_list = [data['height'] for data in stats]
    ...
    height_list = [data['height'] for data in worst_case]
